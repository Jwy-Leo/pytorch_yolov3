...
def main():
    ...
    namefile_path = data_options['names']
    with open(namefile_path,'r') as F:
        lines = F.readlines()
        namelist = [str.split(line,"\n")[0] for line in lines]
...
def test():
    ...
    for data, target, org_w, org_h in test_loader:
        ...
        for k in range(len(all_boxes)):
            ...
            total_c[range(num_classes)]+=np.histogram(truths[:,0],range(num_classes+1))[0]
            ...
            [numberofdata,_]=np.histogram(boxes[Fg_mask,6],range(num_classes+1))
    ...
    logging("Total correct: %d, precision: %f, recall: %f, fscore: %f" % (correct, precision, recall, fscore))
    ...
    for i in range(num_classes):
        ...
        fscore = 2.0 *precision*recall/(precision+recall+eps)
        logging("%s\t correct: %d, precision: %f, recall: %f, fscore: %f" % (namelist[i],correct_c[i], precision, recall, fscore))
if __name__ == '__main__':
    ...
    parser.add_argument('--FB_thresh',
                        '-fb',type=float,default=0.25,help='thresh')
