...
class listDataset(Dataset):
    ...
    def get_different_scale(self):
        ''' wenyen modify
        temp = (self.shape[0]+self.shape[1])/2.0
        receptive_field=int((self.shape[0]+self.shape[1])/2.0)/13
        if temp/13.0-int(temp/13.0) == 0:
            receptive_field = int(temp) / 13
        else:
            receptive_field = int(temp) / 13 + 1
        print(receptive_field)
        '''
        receptive_field = 32
        if self.seen < 4000*64:
            wh = 13*receptive_field                          # 416
        elif self.seen < 8000*64:
            wh = (random.randint(0,3) + 13)*receptive_field  # 416, 480
        elif self.seen < 12000*64:
            wh = (random.randint(0,5) + 12)*receptive_field  # 384, ..., 544
        elif self.seen < 16000*64:
            wh = (random.randint(0,7) + 11)*receptive_field  # 352, ..., 576
        else: # self.seen < 20000*64:
            wh = (random.randint(0,9) + 10)*receptive_field  # 320, ..., 608
        return (wh, wh)
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            if index % 64 == 0: # in paper, every 10 batches, but we did every 64 images
                self.shape = self.get_different_scale() # wenyen modify
