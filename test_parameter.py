...
if __name__ == '__main__':
    ...
    for i in range(10):
        conf_thresh =  i *0.1
        main()
    print("{}".format("".join(["eps {}, precision {}, recall \
                               {}".format(i,precision[i],recall_L[i]) for i in \
                               range(10)])))
    print("Best F/B thresh : \
          {}".format((np.where(np.max(np.array(fscore_L))==np.array(fscore_L))[0]+1)*0.1))
